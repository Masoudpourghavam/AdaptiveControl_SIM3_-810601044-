...
%      u_control(k,1)=(-r_hat0(k,1)*u_control(k-1,1)-s_hat1(k,1)*y_out(k,1)-s_hat0(k,1)*y_out(k-1,1))/r_hat1(k,1);
%     
    for m=1:2
        if k-m<=0
            uf_dummy=0;
    ...
    uf(k,1)=u_control(k,1);
   %  uf(k,1)=u_control(k-1,1)+0.5*uf(k-1,1);
   %  yf(k,1)=y_out(k-1,1)+0.5*yf(k-1,1);
%    uf(k,1)=u_control(k,1)+(1/b0)*u_control(k-1,1)+phi_u*[c1;c0];
%    yf(k,1)=y_out(k,1)+(1/b0)*y_out(k-1,1)+phi_y*[c1;c0];
    yf(k,1)=y_out(k,1);
    for ii=0:1
        if k-ii-d<=0
            y_dummy=0;
...
%     phi_t(1,5)=epsilon(k-1,1);
    
%     gain=(p*(phi_t'))/(landa+phi_t*p*(phi_t'));
%     p=(p-((gain)*phi_t*p))/landa;
...
%     p=p-((p*(phi_t')*phi_t*p)/(1+phi_t*p*(phi_t')));
    ...
    s_hat0(k,1)=teta_hat(4);
     ...
     u_control(k+1,1)=(-r_hat0(k,1)*u_control(k-1,1)-s_hat1(k,1)*y_out(k,1)-s_hat0(k,1)*y_out(k-1,1));

     if abs(u_control(k+1,1))>1000
         u_control(k+1,1)=1000*sign(u_control(k+1,1));
     end
    if k<=1
        V_out(k,1)=y_out(k,1);

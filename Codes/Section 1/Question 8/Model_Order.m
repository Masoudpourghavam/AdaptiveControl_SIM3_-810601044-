...
% Question 1-8 Model otder: Under-parameter
%% --------------------------------------------- %%
...
for k=2:N-1
    ...
    for ii=1:2
     ...
     for jj=2:3
        if k-jj<=0
            ...
            u_dummy_out=u_control(k-jj,1);
            y_dummy_out=-y_out(k-jj,1);
            e_dummy_out=white_noise(k-jj,1);
        end
        phi_out(1,jj-1)=y_dummy_out;
        phi_out(1,jj+1)=u_dummy_out;
        phi_out(1,jj+3)=e_dummy_out;
    end
   ...
   % p=inv(inv(p)+(phi_t')*phi_t);
    gain=p*(phi_t');
    y_out(k,1)=phi_out*teta0+white_noise(k-1,1)-a2*y_out(k-1,1);
    ...
    C_hat=z^2+c_hat1(k,1)*z+c_hat0(k,1);
   
    r1(k,1)=b_hat1(k,1);
    r0(k,1)=b_hat0(k,1);
    
    s1(k,1)=c_hat1(k,1)-a_hat1(k,1);
    s0(k,1)=c_hat0(k,1)-a_hat0(k,1);
    
    if k <=1
        u_control(k,1)=0;
    elseif k>=2
        u_control(k,1)=(-r0(k,1)*u_control(k-1,1)-s1(k,1)*y_out(k,1)-s0(k,1)*y_out(k-1,1))/r1(k,1);
    end
...
subplot(2,1,2)
plot(sample_number,V_control,"black",'LineWidth',1.5)
xlabel('Iter')
...
ylabel('r0')
subplot(2,1,2)
plot(sample_number,r1,"black",'LineWidth',1.5)
xlabel('Iter')
ylabel('r1')
%
figure
